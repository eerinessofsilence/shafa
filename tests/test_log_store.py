...
def test_log_store_rotates_global_and_account_logs(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setenv("SHAFA_MAX_LOG_FILE_BYTES", "1024")
    monkeypatch.setenv("SHAFA_LOG_BACKUP_COUNT", "2")
    store = LogStore(tmp_path / "runtime" / "logs")
    account_log = tmp_path / "accounts" / "acc-1" / "logs" / "app.log"
    message = "x" * 900

    store.append(
        LogRecord(datetime(2026, 4, 13, 12, 0, 0), message, "INFO", "acc-1", "A1"),
        account_log_file=account_log,
    )
    store.append(
        LogRecord(datetime(2026, 4, 13, 12, 1, 0), message, "INFO", "acc-1", "A1"),
        account_log_file=account_log,
    )

    assert (tmp_path / "runtime" / "logs" / "all.log.1").exists()
    assert account_log.with_name("app.log.1").exists()
    assert message in (tmp_path / "runtime" / "logs" / "all.log").read_text(encoding="utf-8")
    assert message in account_log.read_text(encoding="utf-8")
